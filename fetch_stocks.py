...
def fetch_one_ticker(ticker, name, sector, region):
    ...
    try:
        ...
        # El bar mas reciente puede tener close=None (sesion incompleta en el momento
        # de la descarga, frecuente en Europa). Buscamos hacia atras hasta dos cierres validos.
        price = None
        prev_close = None
        for bar in reversed(bars_daily):
            c = bar.get("close")
            if c is None or c != c:  # None o NaN
                continue
            if price is None:
                price = c
